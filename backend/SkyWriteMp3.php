...

{
	...
	if (!empty($TagFormatsToWrite)) {
		...
		if($_REQUEST['cid'] && is_file('../siteuploads/thumb/c/'.$_REQUEST['cid'].'_4.jpg')){
			$thumbImg = '../siteuploads/thumb/c/'.$_REQUEST['cid'].'_4.jpg';
		}
		else
		...
		if ($tagwriter->WriteTags()) {
			...
			*/
			$goto = '/backend/files/create/cid/'.$_REQUEST['cid'].'/lastFile/'.$_REQUEST['lastFile'];
			if($_REQUEST['cid'])
