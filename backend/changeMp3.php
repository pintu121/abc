...
echo '<HTML><HEAD><TITLE>SKYiTech :: MP3 tag writer</TITLE></HEAD><BODY>';
...
if (isset($_POST['WriteTags'])) {
	...
	if (!empty($TagFormatsToWrite)) {
		...
		elseif(is_file('skyTagImage.jpg')) {
			if (in_array('id3v2.4', $tagwriter->tagformats) || in_array('id3v2.3', $tagwriter->tagformats) || in_array('id3v2.2', $tagwriter->tagformats)) {
				{
					if ($fd = @fopen('skyTagImage.jpg', 'rb')) {
						$APICdata = fread($fd, filesize('skyTagImage.jpg'));
						fclose ($fd);

						list($APIC_width, $APIC_height, $APIC_imageTypeID) = GetImageSize('skyTagImage.jpg');
						$imagetypes = array(1=>'gif', 2=>'jpeg', 3=>'png');
						if (isset($imagetypes[$APIC_imageTypeID])) {
							$TagData['attached_picture'][0]['data']          = $APICdata;
							$TagData['attached_picture'][0]['picturetypeid'] = 0;
							$TagData['attached_picture'][0]['description']   = 'skyTagImage.jpg';
							$TagData['attached_picture'][0]['mime']          = 'image/'.$imagetypes[$APIC_imageTypeID];
						} else {
							echo '<B>invalid image format (only GIF, JPEG, PNG)</B><BR>';
						}
...
echo '<H4>SKYiTech :: tag editor/writer</H4>';
...
if (!empty($Filename)) {
	...
	echo '<TR><TD ALIGN="RIGHT"><B>Filename: </B></TD><TD><INPUT TYPE="HIDDEN" NAME="Filename" VALUE="'.FixTextFields($Filename).'">'.$Filename.'</TD></TR>';
