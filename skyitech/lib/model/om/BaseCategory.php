...
abstract class BaseCategory extends BaseObject  implements Persistent {
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->ord = $rs->getInt($startcol + 8);

			$this->flag_new = $rs->getInt($startcol + 9);

			$this->flag_updated = $rs->getInt($startcol + 10);

			$this->flag_hot = $rs->getInt($startcol + 11);

			$this->files = $rs->getInt($startcol + 12);
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[8] => $this->getOrd(),
			$keys[9] => $this->getFlagNew(),
			$keys[10] => $this->getFlagUpdated(),
			$keys[11] => $this->getFlagHot(),
			$keys[12] => $this->getFiles(),
			$keys[13] => $this->getUrl(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[9], $arr)) $this->setFlagNew($arr[$keys[9]]);
		if (array_key_exists($keys[10], $arr)) $this->setFlagUpdated($arr[$keys[10]]);
		if (array_key_exists($keys[11], $arr)) $this->setFlagHot($arr[$keys[11]]);
		if (array_key_exists($keys[12], $arr)) $this->setFiles($arr[$keys[12]]);
