...
abstract class BaseFiles extends BaseObject  implements Persistent {
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->today = $rs->getInt($startcol + 5);

			$this->download = $rs->getInt($startcol + 6);

			$this->extension = $rs->getString($startcol + 7);
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[4] => $this->getSize(),
			$keys[5] => $this->getToday(),
			$keys[6] => $this->getDownload(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[5], $arr)) $this->setToday($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setDownload($arr[$keys[6]]);
