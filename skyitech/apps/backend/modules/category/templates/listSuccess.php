...
<tr class="sortable <?php echo $category->getChild(); ?>" id="item_<?php echo $category->getId();?>">
	...
	<td><?php 
			if(is_file(sfConfig::get('sf_upload_dir').'/thumb/c/'.$category->getId().'_2.jpg'))
					echo image_tag(sfConfig::get('app_upload_dir').'/thumb/c/'.$category->getId().'_2.jpg');
		?></td>
	<td>
		<?php
...
<?php
	...
	$pageNum = myUser::pageNavigate($pager,sfConfig::get('app_webpath').'/category/list'.$parent.'/page/');
	if($pageNum){
?>
	<div class="pgn">
		<?php echo $pageNum; ?>
