...
class categoryActions extends sfActions
{
  ...
  {
/**
		if(!stristr($_SERVER['HTTP_HOST'],'mirchiloft.com'))
			if(!stristr($_SERVER['HTTP_HOST'],'mirchiloft.sky'))
				exit;
**/
  	...
  	elseif($this->getRequestParameter('newupdatedhot')):
	    ...
	    $pager = new sfPropelPager('Category', 10);
	    $pager->setCriteria($c);
  ...
  public function executeMakeDir()
  {
		$connection = Propel::getConnection();
		$statement = $connection->prepareStatement("SHOW TABLE STATUS LIKE 'category'");
		$resultset = $statement->executeQuery();
		$resultset->next();
		$startID = $resultset->getInt('Auto_increment');
		$startID--;
		$parent = $this->getRequestParameter('parent');
		$folder = $this->getRequestParameter('folder');
    $category = CategoryPeer::retrieveByPk($this->getRequestParameter('parent'));
		
		//$this->redirect('/makeDir.php?parent='.$parent.'&lastID='.$lastID.'&folder='.$folder);
		header('Location: /backend/makeDir.php?parent='.$parent.'&startID='.$startID.'&parents='.$category->getParents().'&folder='.$folder);
		exit;
  }
  ...
  public function executeUpdate()
  {
    ...
    if (!$this->getRequestParameter('id'))
    {
       $category = new Category();
	    ...
	    if($this->getRequestParameter('parentsarray'))
    ...
    if($this->getRequestParameter('dirCatch')==true){
	    ...
	    $category->setFlagHot(0);
   		 //if($parentCategory->getParentsarray())
	    	$category->setParentsarray($parentCategory->getParentsarray().'||'.$this->getRequestParameter('parent').'||'.$parentCategory->getCategoryName());
    }
    else{
	    ...
	    $category->setOrd($this->getRequestParameter('ord'));
		...
		*/
		...
		if($this->getRequest()->getFileSize('thumb_name') > 0)
		{
...

			{
				...
				myClass::getImageRatio($this->getRequest()->getFilePath('thumb_name'),'thumb/c/',60, 70, $width1, $height1,false, $category->getId().'_1.jpg',true,95,'logo2.png');
				myClass::getImageRatio($this->getRequest()->getFilePath('thumb_name'),'thumb/c/',100, 100, $width1, $height1,false, $category->getId().'_2.jpg',true,95,'logo.png');
				myClass::getImageRatio($this->getRequest()->getFilePath('thumb_name'),'thumb/c/',150, 150, $width1, $height1,false, $category->getId().'_3.jpg',true,95,'logo.png',$position);
				myClass::getImageRatio($this->getRequest()->getFilePath('thumb_name'),'thumb/c/',250, 250, $width1, $height1,false, $category->getId().'_4.jpg',true,95,'logo5.png',$position);
			}
			$this->getRequest()->moveFile('thumb_name', sfConfig::get('sf_upload_dir').'/thumb/c/'.$category->getId().'_0.jpg');
		}
		elseif($this->getRequestParameter('position_force')=='yes'){
				myClass::getImageRatio(sfConfig::get('sf_upload_dir').'/thumb/c/'.$category->getId().'_0.jpg','thumb/c/',150, 150, $width1, $height1,false, $category->getId().'_3.jpg',true,95,'logo.png',$position);
				myClass::getImageRatio(sfConfig::get('sf_upload_dir').'/thumb/c/'.$category->getId().'_0.jpg','thumb/c/',250, 250, $width1, $height1,false, $category->getId().'_4.jpg',true,95,'logo5.png',$position);
		}
		if($this->getRequestParameter('dirCatch')==true){
  ...
  public function executeActivation()
  {
    ...
    $c->save();
  	if($this->getRequestParameter('status') == 'B')
  		echo '<a onclick="new Ajax.Updater(\'active_'.$this->getRequestParameter('id').'\', \'/backend/category/activation/id/'.$this->getRequestParameter('id').'/status/A\', {asynchronous:true, evalScripts:false});; return false;" href="#">Active</a>';
  	else
  		echo '<a onclick="new Ajax.Updater(\'active_'.$this->getRequestParameter('id').'\', \'/backend/category/activation/id/'.$this->getRequestParameter('id').'/status/B\', {asynchronous:true, evalScripts:false});; return false;" href="#"><span class="red">Block</span></a>';
		return sfView::NONE;
  }
